...
import { ref } from 'vue';
import { getMonthlyIndicators } from '@/services/testes.service';

export function useTestes() {
  console.log('useTestes  step1');
  ...
  const loadTestes = async () => {
    console.log('loadTestes called  step2');
    
    loading.value = true;
    error.value = null;
    try {
      const response = await getMonthlyIndicators();
      console.log('response from getMonthlyIndicators step3:', response);
      testes.value = response.indicators ?? response;
